# Part 5- CCA Loop & surrounding areas population trend analysis
# May 11, 2020
...
def get_filters():
    print("Hello, Lets explore the CTA L-ridership data")
    while True:
        line = input(
            "Hello! Which line do you want to analyze? RED, BLUE, G, BRN, P, Pexp, Y, Pnk, or O?\n"
        )
        if line not in ("RED", "BLUE", "G", "BRN", "P", "Pexp", "Y", "Pnk", "O"):
...
def load_data():
    ...
    l_map_stops = pd.read_csv(
        "https://data.cityofchicago.org/resource/zbnc-zirh.csv", sep=","
    )
...
stationa = merged_left[merged_left["STOP_ID"].isin(["NaN"])].copy()
...
# Accessed the Chicago Census Data File containing the CCA values (chi_community_areas) and population data
# Home Drive Users Location: /Users/shilpakamath-jha/ReferenceCCAProfiles20132017WITHCCAVALUES.csv

chi_cca_census = pd.read_csv(
...
chi_cca_census = pd.read_csv(
    "/Users/shilpakamath-jha/ReferenceCCAProfiles20132017WITHCCAVALUES.csv", sep=","
)
chi_cca_census.sort_values(ascending=True, by="chi_community_areas", inplace=True)
...
cca_data
pop_change_cca = cca_data.query(
    "chi_community_areas == [6,7,8,22,18,31,32,33,34,35]"
).copy()
pop_change_cca.sort_values(ascending=False, by="percent_diff_2017_2010", inplace=True)
pop_change_cca
